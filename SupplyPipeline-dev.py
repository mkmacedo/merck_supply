...
class Medicamentos:
    ...
    def calcular(self, month):
        ...
        for i in range(len(indexes)):        
            if indexes[i] == month:
                beginning = i
                forecastLimiter = len(indexes) - i



                # if len(indexes) - 4 <= len(JDA_Cols) - 14:
                #     print(len(indexes))
                #     limit = len(indexes) - 1
                #     limiter = len(indexes) - beginning

                # else:
                #     print(len(indexes))
                #     limit = beginning + len(JDA_Cols) - 15
                #     limiter = len(JDA_Cols) - 15
 
                #limit = beginning + min(len(indexes) - 5, len(JDA_Cols) - 16)
        ...
        # jdaLimit = jdaBeginning + min(len(indexes) - 5, len(JDA_Cols) - 16)
        jdaLimiter = len(JDA_Cols) - jdaBeginning
        limiter = jdaLimiter = min(forecastLimiter, jdaLimiter)

        limit = beginning + limiter
        jdaLimit = jdaBeginning + limiter

        # tempSeries = tempSeries[JDA_Cols[jdaBeginning:jdaLimit]]

        print(indexes[beginning:beginning + forecastLimiter])
        print(JDA_Cols[jdaBeginning:jdaLimit])

        print(beginning, limit)
        print(jdaBeginning, jdaLimit)
        ...
        for i in range(len(self.df_forecast)):
            ...
            if code in codes:
                ...
                df[code]['Material No'] = [code] * (limit - beginning + 1)
