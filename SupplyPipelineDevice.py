...
class Medicamentos:
    ...
    def calcular(self, month):
        ...
        for i in range(len(self.df_forecast)):
            ...
            #print(self.df_forecast.loc[i, 'Key Figure'])

            if code in codes:
                print('Entrou codes') #Debug
                ...
                if self.df_forecast.loc[i, 'Key Figure'] == 'AGV Forecast':
                    ...
                    print('AGV', code) #debug

                elif self.df_forecast.loc[i, 'Key Figure'] == 'DIREMADI Forecast':
                    df[code]['DIREMADI Forecast'] = forecast[list(df[code]['Meses'])].values
                    print('DIREMADI', code) #debug

                df[code]['Forecast'] = df[code].apply(lambda x: 0., axis = 1)
        ...
        for k in df:
            series1 = pd.to_numeric(df[k]['AGV Forecast'], errors='coerce')
            series2 = pd.to_numeric(df[k]['DIREMADI Forecast'], errors='coerce')
            df[k]['Forecast'] = series1 + series2
            print(df[k]['Forecast']) #debug
